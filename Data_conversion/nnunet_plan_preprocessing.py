...
def plan_and_preprocess():
    ...
    # command = "nnUNetv2_plan_and_preprocess -d 7 -pl ExperimentPlanner -c 3d_fullres -np 2 --verify_dataset_integrity"

    '''Run the following commands for new data, if you want to use pre-trained model, step 1:'''
    command = ("nnUNetv2_move_plans_between_datasets -s 4 -t 7 "
               "-sp plans -tp nnUNetPlans_finetune")

    '''Run the following commands for new data, if you want to use pre-trained model, step 2:'''
    # command = ("nnUNetv2_extract_fingerprint -d 7")
    command = ("nnUNetv2_preprocess -d 7 -plans_name nnUNetPlans_finetune "
